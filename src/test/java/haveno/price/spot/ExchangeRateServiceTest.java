...
public class ExchangeRateServiceTest {
    ...
    public void getAllMarketPrices_withNoExchangeRates_logs_Exception() {
        ...
        // This allows Haveno clients to eventually disregard data from this provider
    ...
    public void getAllMarketPrices_withMultipleProviders_differentCurrencyCodes() {
        ...
        checkHavenoIndexCalculationNoOutliers(validateAndGetRetrievedRates(retrievedData), providers);
    ...
    public void getAllMarketPrices_oneProvider_considerBlueUpdates() {
        ...
        when(mockedEnvironment.getProperty(eq("haveno.price.cryptocurrency.excluded"), anyString())).thenReturn("");
        when(mockedEnvironment.getProperty(eq("haveno.price.fiatcurrency.excluded"), anyString())).thenReturn(excludedCcvString);
        when(mockedEnvironment.getProperty(eq("haveno.price.fiatcurrency.excludedByProvider"), anyString())).thenReturn(providerExcludedCcvString);
    ...
    @Test
    public void havenoIndexCalculation_oneOutlierPriceWideRange() {
        String fiatCoin = "BRL";
        List<ExchangeRateProvider> providers = asList(
        ...
        Map<String, Object> retrievedData = new ExchangeRateService(new StandardEnvironment(), providers, Collections.emptyList()).getAllMarketPrices();
        doSanityChecksForRetrievedDataMultipleProviders(retrievedData, providers);
        checkHavenoIndexCalculationWithOutlier(validateAndGetRetrievedRates(retrievedData), providers, Collections.singletonList("mercadoBitcoin"));
    }

    @Test
    public void havenoIndexCalculation_oneOutlierPriceCloseRange() {
        String fiatCoin = "BRL";
        List<ExchangeRateProvider> providers = asList(
        ...
        Map<String, Object> retrievedData = new ExchangeRateService(new StandardEnvironment(), providers, Collections.emptyList()).getAllMarketPrices();
        doSanityChecksForRetrievedDataMultipleProviders(retrievedData, providers);
        checkHavenoIndexCalculationWithOutlier(validateAndGetRetrievedRates(retrievedData), providers, Collections.singletonList("coinGecko"));
    }

    @Test
    public void havenoIndexCalculation_twoOutlierPrices() {
        String fiatCoin = "GBP";
        List<ExchangeRateProvider> providers = asList(
        ...
        Map<String, Object> retrievedData = new ExchangeRateService(new StandardEnvironment(), providers, Collections.emptyList()).getAllMarketPrices();
        doSanityChecksForRetrievedDataMultipleProviders(retrievedData, providers);
        checkHavenoIndexCalculationWithOutlier(validateAndGetRetrievedRates(retrievedData), providers, List.of("binance", "bitfinex"));
    }

    @Test
    public void havenoIndexCalculation_altcoinOutlierPrices() {
        String altcoin = "ETH";
        List<ExchangeRateProvider> providers = asList(
        ...
        Map<String, Object> retrievedData = new ExchangeRateService(new StandardEnvironment(), providers, Collections.emptyList()).getAllMarketPrices();
        doSanityChecksForRetrievedDataMultipleProviders(retrievedData, providers);
        checkHavenoIndexCalculationWithOutlier(validateAndGetRetrievedRates(retrievedData), providers, List.of("binance", "poloniex"));
    }

    @Test
    public void havenoIndexCalculation_allSamePricesAltcoin() {
        String altcoin = "DOGE";
        List<ExchangeRateProvider> providers = asList(
    ...
    @Test
    public void havenoIndexCalculation_allSamePricesFiat() {
        String fiat = "USD";
        List<ExchangeRateProvider> providers = asList(
    ...
    public void getAllMarketPrices_withMultipleProviders_excludedCurrencyCodes() {
        ...
        Environment mockedEnvironment = mock(Environment.class);
        when(mockedEnvironment.getProperty(eq("haveno.price.fiatcurrency.excluded"), anyString())).thenReturn(excludedCcyString);
        when(mockedEnvironment.getProperty(eq("haveno.price.fiatcurrency.excludedByProvider"), anyString())).thenReturn(providerExcludedCcyString);
    ...
    private List<ExchangeRate> validateAndGetRetrievedRates(Map<String, Object> retrievedData) {
        ...
        Set<List<String>> seenPairs = new HashSet<List<String>>();
        for (ExchangeRate retrievedRate : retrievedRates) {
            List<String> pair = Arrays.asList(retrievedRate.getBaseCurrency(), retrievedRate.getCounterCurrency());
            assertFalse(seenPairs.contains(pair), "Found duplicate pair " + pair);
            seenPairs.add(pair);
        }
    ...
    private void checkHavenoIndexCalculationNoOutliers(List<ExchangeRate> retrievedData,
                                                      List<ExchangeRateProvider> providers) {
        checkHavenoIndexCalculation(retrievedData, providers, Collections.emptyList());
    }

    private void checkHavenoIndexCalculationWithOutlier(List<ExchangeRate> retrievedData,
                                           List<ExchangeRateProvider> providers,
                                           List<String> outlierProviderNames) {
        checkHavenoIndexCalculation(retrievedData, providers, outlierProviderNames);
    }

    // checks that the haveno index equals the average of all retrieved quotes
    private void checkHavenoIndexCalculation(List<ExchangeRate> retrievedRates,
                                           List<ExchangeRateProvider> providers,
        ...
        for (ExchangeRate retrievedRate : retrievedRates) {
            ...
            if (currencyCodeToExchangeRateFromService.containsKey(otherCurrency)) throw new RuntimeException("Found duplicate rates for currency code " + otherCurrency);
        ...
        currencyCodeToExchangeRatesFromProviders.forEach((currencyCode, exchangeRateList) -> {
            ExchangeRate actualHavenoIndex = currencyCodeToExchangeRateFromService.get(currencyCode);
            OptionalDouble testHavenoIndex = exchangeRateList.stream()
            ...
            assertEquals(testHavenoIndex.getAsDouble(), actualHavenoIndex.getPrice(), "Service returned incorrect aggregate rate");
        });
    }
    ...
    private ExchangeRateProvider buildDummyExchangeRateProviderWithRateAndTimestamp(
        ...
        ExchangeRateProvider dummyProvider = new ExchangeRateProvider(
            ...
            protected Set<ExchangeRate> doGet() {
                ...
                exchangeRates.add(new ExchangeRate(
                        isFiat ? "XMR" : currencyCode,
                        isFiat ? currencyCode : "XMR",
                        rate,
                        timestamp,
                        getName())); // ExchangeRateProvider name
                return exchangeRates;
